...
function monthDay(value:unknown){return String(value||'').slice(5,10)}
...
async function googleAccessToken(service:any){
  ...
  const response=await fetch(service.token_uri||'https://oauth2.googleapis.com/token',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({grant_type:'urn:ietf:params:oauth-token-type:jwt-bearer',assertion:`${unsigned}.${b64url(signature)}`})});
...
Deno.serve(async(req:Request)=>{
  ...
  const today=localDate(),todayMonthDay=monthDay(today);
  const {data:birthdayRows,error:eventError}=await admin.from('calendar_entries')
    .select('id,family_id,title,entry_date,person_id,created_at')
    .eq('calendar_context','plan')
    .eq('kind','event')
    .eq('event_type','birthday')
    .order('created_at',{ascending:true});
  if(eventError)return json({ok:false,error:eventError.message},500);
  const events=(birthdayRows||[]).filter((event:any)=>monthDay(event.entry_date)===todayMonthDay);
  if(!events.length)return json({ok:true,date:today,month_day:todayMonthDay,families:0,sent:0});
  ...
  for(const [familyId,items] of grouped){
    ...
    const {data:devices,error:deviceError}=await admin.from('push_devices').select('id,token,user_id').eq('family_id',familyId).eq('enabled',true);
    ...
    for(const device of devices){
      const response=await sendToToken(service,accessToken,device.token,'🎂 День рождения сегодня',body,{kind:'birthday',date:today,month_day:todayMonthDay,family_id:familyId});
    ...
    if(familySent>0){
      await admin.from('birthday_push_deliveries').insert({family_id:familyId,occurrence_date:today,event_ids:items.map(item=>item.id),sent_devices:familySent});
    }
  }
  return json({ok:true,date:today,month_day:todayMonthDay,families:grouped.size,sent,failed,skipped});
});