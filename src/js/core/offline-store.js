...
(function(){
  ...
  async function restoreSnapshot(userId=uid()){if(!userId)return false;try{const saved=await storeGet(SNAPSHOT_STORE,snapshotKey(userId));if(!saved?.data)return false;for(const key of SNAPSHOT_FIELDS)if(Object.prototype.hasOwnProperty.call(saved.data,key))state[key]=saved.data[key];state.offlineSnapshotAt=saved.savedAt||null;await reapplyPendingToState();updateStatus();if(typeof renderApp==='function')renderApp();return true}catch(error){console.warn('Не удалось открыть офлайн-копию',error);return false}}
  ...
  async function enqueueUpdate(id,payload){
    ...
    const item=existing?{...existing,payload:{...existing.payload,...payload},lastError:'',conflictData:null}:queueItem('update',id,payload,{baseUpdatedAt:current?.updated_at||null});await storePut(QUEUE_STORE,item);const row={...current,...payload,updated_at:nowIso(),updated_by:uid(),_offline:true,_pending_action:'update'};syncTransactionState(row);await refreshPendingCount();persistSnapshotSoon();return row;
  }
  async function enqueueDeletedState(id,deleted){
    ...
    const current=byId(state.transactions,id)||byId(state.trashTransactions,id),value=deleted?nowIso():null,item=queueItem(deleted?'delete':'restore',id,{deleted_at:value},{baseUpdatedAt:current?.updated_at||null});await storePut(QUEUE_STORE,item);const row={...current,deleted_at:value,deleted_by:deleted?uid():null,updated_at:nowIso(),updated_by:uid(),_offline:true,_pending_action:deleted?'delete':'restore'};syncTransactionState(row);await refreshPendingCount();persistSnapshotSoon();return{removed:false,data:row};
  }
  ...
  function entityOptimistic(entity,payload,id,action='create'){const stamp=nowIso(),base={id,...payload,_offline:true,_pending_action:action};if(entity==='goal')return{created_at:stamp,archived:false,...base};if(entity==='contribution')return{contributed_at:stamp,...base};if(entity==='category')return{is_system:false,sort_order:999,...base};if(entity==='subcategory')return{sort_order:999,...base};return{created_at:stamp,updated_at:stamp,...base}}
  ...
  function stripLocalFields(value){if(Array.isArray(value))return value.map(stripLocalFields);if(value&&typeof value==='object'){const out={};for(const key of Object.keys(value).sort()){if(key==='_offline'||key==='_pending_action')continue;out[key]=stripLocalFields(value[key])}return out}return value}
  function entitySnapshot(row){return row?stripLocalFields(row):null}
  function sameSnapshot(a,b){return JSON.stringify(entitySnapshot(a))===JSON.stringify(entitySnapshot(b))}
  ...
  async function enqueueEntityUpdate(entity,id,payload){
    ...
    const queue=await ownQueue(),existing=[...queue].reverse().find(x=>x.domain==='entity'&&x.entity===entity&&x.kind==='update'&&x.targetId===id),item=existing?{...existing,payload:{...existing.payload,...payload},lastError:'',conflictData:null}:queueItem('update',id,payload,{domain:'entity',entity,baseSnapshot:entitySnapshot(current)});await storePut(QUEUE_STORE,item);const row={...current,...payload,_offline:true,_pending_action:'update'};applyEntityRow(entity,row);await refreshPendingCount();return row;
  }
  async function enqueueEntityDelete(entity,id){
    ...
    await storePut(QUEUE_STORE,queueItem('delete',id,{}, {domain:'entity',entity,baseSnapshot:entitySnapshot(current)}));state[ENTITY_CONFIG[entity].stateKey]=list.filter(x=>x.id!==id);await refreshPendingCount();persistSnapshotSoon();return true;
  }
  ...
  async function transactionConflict(item){if(item.force||!item.baseUpdatedAt||item.kind==='create')return null;const targetId=item.targetId||item.transactionId;if(isTempId(targetId))return null;const {data,error}=await sb.from('transactions').select('*').eq('id',targetId).single();if(error)return{error};const serverTime=new Date(data.updated_at||data.created_at||0).getTime(),baseTime=new Date(item.baseUpdatedAt||0).getTime();if(serverTime>baseTime+500)return{serverData:data};return null}
  async function entityConflict(item){if(item.force||item.kind==='create'||isTempId(item.targetId))return null;const cfg=ENTITY_CONFIG[item.entity];if(!cfg)return{error:new Error('Неизвестный тип очереди')};const {data,error}=await sb.from(cfg.table).select('*').eq('id',item.targetId).maybeSingle();if(error)return{error};if(!data)return item.kind==='delete'?{alreadyGone:true}:{serverData:null,missing:true};if(item.baseSnapshot&&!sameSnapshot(data,item.baseSnapshot))return{serverData:data};return null}
  async function processTransactionItem(item){
    const targetId=item.targetId||item.transactionId;let result;if(item.kind==='create'){const p=item.payload;result=await sb.rpc('create_family_transaction',{p_family_id:item.familyId,p_person_id:p.person_id,p_type:p.type,p_amount:p.amount,p_category_id:p.category_id,p_subcategory_id:p.subcategory_id||null,p_description:p.description||null,p_occurred_at:p.occurred_at});if(!result.error){const row=Array.isArray(result.data)?result.data[0]:result.data;state.transactions=state.transactions.filter(x=>x.id!==targetId);state.trashTransactions=state.trashTransactions.filter(x=>x.id!==targetId);if(row)syncTransactionState(row)}return result}const conflict=await transactionConflict(item);if(conflict?.error)return{error:conflict.error};if(conflict?.serverData)return{error:null,conflict:true,serverData:conflict.serverData};const payload=item.kind==='update'?item.payload:{deleted_at:item.kind==='delete'?(item.payload.deleted_at||item.queuedAt):null};result=await sb.from('transactions').update(payload).eq('id',targetId).select().single();if(!result.error&&result.data)syncTransactionState(result.data);return result;
  }
  async function processEntityItem(item){
    const cfg=ENTITY_CONFIG[item.entity];if(!cfg)return{error:new Error('Неизвестный тип очереди')};let result;if(item.kind==='create')result=await sb.from(cfg.table).insert(item.payload).select().single();else{const conflict=await entityConflict(item);if(conflict?.error)return{error:conflict.error};if(conflict?.alreadyGone){state[cfg.stateKey]=state[cfg.stateKey].filter(x=>x.id!==item.targetId);return{error:null,data:null}}if(Object.prototype.hasOwnProperty.call(conflict||{},'serverData'))return{error:null,conflict:true,serverData:conflict.serverData,entity:item.entity};if(item.kind==='update')result=await sb.from(cfg.table).update(item.payload).eq('id',item.targetId).select().single();else result=await sb.from(cfg.table).delete().eq('id',item.targetId)}if(!result.error&&result.data){if(item.kind==='create'&&isTempId(item.targetId))await remapTempId(item.targetId,result.data.id);applyEntityRow(item.entity,result.data)}if(!result.error&&item.kind==='delete')state[cfg.stateKey]=state[cfg.stateKey].filter(x=>x.id!==item.targetId);return result;
  }
  async function processItem(item){return item.domain==='entity'?processEntityItem(item):processTransactionItem(item)}
  async function flushQueue(){
    if(syncing||!navigator.onLine||!uid()||!state.family)return 0;syncing=true;lastSyncError='';let synced=0;try{const queue=await ownQueue();for(const item of queue){try{const result=await processItem(item);if(result?.conflict){item.conflictData={serverData:result.serverData,detectedAt:nowIso()};item.lastError='Конфликт: запись изменилась на другом устройстве';await storePut(QUEUE_STORE,item);lastSyncError=item.lastError;break}if(result?.error){if(networkError(result.error)){lastSyncError=result.error.message||'Нет сети';break}item.lastError=result.error.message||String(result.error);await storePut(QUEUE_STORE,item);lastSyncError=item.lastError;break}await storeDelete(QUEUE_STORE,item.id);synced++}catch(error){if(networkError(error)){lastSyncError=error?.message||'Нет сети';break}item.lastError=error?.message||String(error);await storePut(QUEUE_STORE,item);lastSyncError=item.lastError;break}}await refreshPendingCount();await persistSnapshot();return synced}finally{syncing=false;updateStatus()}
  }
  function queueLabel(item){if(item.domain==='entity'){const noun=ENTITY_CONFIG[item.entity]?.label||'Запись';if(item.kind==='create')return`Создание · ${noun}`;if(item.kind==='delete')return`Удаление · ${noun}`;return`Изменение · ${noun}`}if(item.kind==='create')return'Новая операция';if(item.kind==='delete')return'Удаление операции';if(item.kind==='restore')return'Восстановление операции';return'Изменение операции'}
  function conflictText(item){if(!item.conflictData)return'';if(item.conflictData.serverData===null)return'На другом устройстве запись уже удалена.';return'На другом устройстве эта запись была изменена после вашей офлайн-копии.'}
  async function openSyncCenter(){
    const queue=await ownQueue();closeModal();document.body.insertAdjacentHTML('beforeend',`<div class="modal-backdrop" id="modal"><div class="modal sync-center-modal"><div class="modal-head"><div><h2>Синхронизация</h2><p class="quick-amount-context">${navigator.onLine?'Интернет доступен':'Сейчас нет подключения'}</p></div><button class="icon-btn" id="closeModal">×</button></div><div class="sync-center-actions"><button class="btn btn-primary" id="syncNow" ${!navigator.onLine||syncing?'disabled':''}>Синхронизировать сейчас</button></div><div class="sync-center-list">${queue.length?queue.map(item=>`<div class="sync-item ${item.conflictData?'has-conflict':''}"><div><b>${esc(queueLabel(item))}</b><small>${new Date(item.queuedAt).toLocaleString('ru-RU')}</small>${item.conflictData?`<p>${esc(conflictText(item))}</p>`:item.lastError?`<p>${esc(item.lastError)}</p>`:''}</div>${item.conflictData?`<div class="sync-conflict-actions"><button class="btn btn-soft btn-small syncServerWins" data-id="${item.id}">Оставить серверную</button><button class="btn btn-primary btn-small syncLocalWins" data-id="${item.id}">Отправить мою</button></div>`:''}</div>`).join(''):'<div class="empty">Очередь пуста. Все изменения синхронизированы.</div>'}</div></div></div>`);document.getElementById('closeModal').onclick=closeModal;document.getElementById('modal').onclick=e=>{if(e.target.id==='modal')closeModal()};const sync=document.getElementById('syncNow');if(sync)sync.onclick=async()=>{sync.disabled=true;await flushQueue();openSyncCenter()};document.querySelectorAll('.syncServerWins').forEach(button=>button.onclick=()=>resolveConflict(button.dataset.id,'server'));document.querySelectorAll('.syncLocalWins').forEach(button=>button.onclick=()=>resolveConflict(button.dataset.id,'local'));
  }
  async function resolveConflict(itemId,strategy){
    const item=(await ownQueue()).find(x=>x.id===itemId);if(!item?.conflictData)return;if(strategy==='server'){const server=item.conflictData.serverData;if(item.domain==='entity'){const cfg=ENTITY_CONFIG[item.entity];if(server)applyEntityRow(item.entity,server);else if(cfg)state[cfg.stateKey]=state[cfg.stateKey].filter(x=>x.id!==item.targetId)}else if(server)syncTransactionState(server);else{const id=item.targetId||item.transactionId;state.transactions=state.transactions.filter(x=>x.id!==id);state.trashTransactions=state.trashTransactions.filter(x=>x.id!==id)}await storeDelete(QUEUE_STORE,item.id);await refreshPendingCount();await persistSnapshot();openSyncCenter();renderStateChange();return}item.force=true;item.conflictData=null;item.lastError='';await storePut(QUEUE_STORE,item);await refreshPendingCount();await flushQueue();openSyncCenter();renderStateChange();
  }
