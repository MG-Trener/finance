...
(function(){
  ...
  async function createConfig(pin){
    ...
    return {salt:saltB64,hash,createdAt:new Date().toISOString(),failed:0,blockedUntil:0};
  }
  ...
  function renderLock(){
    ...
    form.onsubmit=async e=>{
      ...
      unlockedUserId=userId();const resolve=pendingUnlock;pendingUnlock=null;resolve?.(true);
    };
    input.addEventListener('input',()=>{input.value=input.value.replace(/\D/g,'').slice(0,6)});
    document.getElementById('localLockSignout').onclick=async()=>{pendingUnlock?.(false);pendingUnlock=null;unlockedUserId=null;await sb.auth.signOut()};
  ...
  window.FinanceLocalLock={enabled,unlockIfNeeded,setPin,disablePin,openSettings,lockNow(){unlockedUserId=null;return unlockIfNeeded(state.user)},reset(){unlockedUserId=null}};
