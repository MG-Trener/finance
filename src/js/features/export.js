// Export current journal selection to CSV or Excel. XLSX is loaded only when requested.
(function(){
  ...
  function exportCSV(){const rows=exportRows();if(!rows.length)return alert('В текущей выборке нет операций для экспорта.');const headers=Object.keys(rows[0]),lines=[headers.map(csvEscape).join(';')];rows.forEach(row=>lines.push(headers.map(h=>csvEscape(row[h])).join(';')));downloadBlob('\uFEFF'+lines.join('\r\n'),'text/csv;charset=utf-8;',`${fileBase()}.csv`)}
