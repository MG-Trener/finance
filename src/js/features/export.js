...
(function(){
  ...
  function exportRows(){
    ...
    return tx.map(x=>{const d=new Date(x.occurred_at),transfer=x.type==='transfer';return{
      'Дата':d.toLocaleDateString('ru-RU'),
      'Время':d.toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'}),
      'Кто / от кого':personName(x.person_id),
      'Кому':transfer?personName(x.transfer_to_person_id):'',
      'Тип':typeof transactionTypeLabel==='function'?transactionTypeLabel(x.type):(x.type==='income'?'Доход':x.type==='expense'?'Расход':'Перевод'),
      'Категория':transfer?'Перевод между супругами':catName(x.category_id),
      'Подкатегория':transfer?'':subName(x.subcategory_id)||'',
      'Сумма, ₸':Number(x.amount)||0,
      'Комментарий':x.description||''
    }})
  }
  function filterDescription(){const f=state.filters||{},parts=[];if((f.period||'month')==='month')parts.push(`${MONTHS[state.month-1]} ${state.year}`);else if(f.period==='year')parts.push(`${state.year} год`);else parts.push('за всё время');if(f.person&&f.person!=='all')parts.push(personName(f.person));if(f.type&&f.type!=='all')parts.push(f.type==='income'?'только доходы':f.type==='expense'?'только расходы':'только переводы');if(f.category&&f.category!=='all')parts.push(catName(f.category));if((f.search||'').trim())parts.push(`поиск: ${f.search.trim()}`);return parts.join(' · ')}
  function summaryRows(){
    const tx=typeof journalFilteredTx==='function'?journalFilteredTx():[],income=tx.filter(x=>x.type==='income').reduce((s,x)=>s+Number(x.amount||0),0),expense=tx.filter(x=>x.type==='expense').reduce((s,x)=>s+Number(x.amount||0),0),transferCount=tx.filter(x=>x.type==='transfer').length;
    const rows=[['Семья',state.family?.name||''],['Фильтр',filterDescription()],['Количество операций',tx.length],['Доходы, ₸',income],['Расходы, ₸',expense],['Семейный баланс, ₸',income-expense],['Переводы между супругами',transferCount],[],['По участникам','','',''],['Участник','Доходы, ₸','Расходы, ₸','Баланс с переводами, ₸']];
    state.people.forEach(p=>{const own=tx.filter(x=>x.person_id===p.id),inc=own.filter(x=>x.type==='income').reduce((s,x)=>s+Number(x.amount||0),0),exp=own.filter(x=>x.type==='expense').reduce((s,x)=>s+Number(x.amount||0),0);let transferIn=0,transferOut=0;tx.filter(x=>x.type==='transfer').forEach(x=>{if(x.transfer_to_person_id===p.id)transferIn+=Number(x.amount||0);if(x.person_id===p.id)transferOut+=Number(x.amount||0)});rows.push([p.display_name,inc,exp,inc-exp+transferIn-transferOut])});return rows
  }
  ...
  async function exportExcel(){
    ...
    const wb=XLSXLib.utils.book_new(),ws=XLSXLib.utils.json_to_sheet(rows);ws['!cols']=[{wch:12},{wch:8},{wch:18},{wch:18},{wch:12},{wch:24},{wch:22},{wch:14},{wch:36}];XLSXLib.utils.book_append_sheet(wb,ws,'Операции');const summary=XLSXLib.utils.aoa_to_sheet(summaryRows());summary['!cols']=[{wch:28},{wch:34},{wch:18},{wch:22}];XLSXLib.utils.book_append_sheet(wb,summary,'Сводка');
