...
function journalFilteredTx(){
  ...
  if(f.person&&f.person!=='all')tx=tx.filter(x=>x.person_id===f.person||x.type==='transfer'&&x.transfer_to_person_id===f.person);
  ...
  if(q)tx=tx.filter(x=>[x.type==='transfer'?'Перевод между супругами':catName(x.category_id),subName(x.subcategory_id),personName(x.person_id),x.transfer_to_person_id?personName(x.transfer_to_person_id):'',x.description||'',String(x.amount)].join(' ').toLowerCase().includes(q));
...
async function permanentlyDeleteTrashTransaction(id,button){
  ...
  const label=[tx.type==='transfer'?'Перевод':catName(tx.category_id),money(tx.amount)].filter(Boolean).join(' · ');
...
function operationsPage(){
  ...
  const tx=journalFilteredTx(),shown=tx.slice(0,state.journalLimit||50),expense=tx.filter(x=>x.type==='expense').reduce((a,x)=>a+Number(x.amount),0),income=tx.filter(x=>x.type==='income').reduce((a,x)=>a+Number(x.amount),0),transfers=tx.filter(x=>x.type==='transfer').length,activeFilters=journalActiveFilterCount(f),filtersOpen=!!f.filtersOpen;
  ...
  return `<div class="page-head journal-head"><div><h2 class="page-title">${f.trash?'Корзина операций':'Журнал операций'}</h2><div class="page-subtitle">${f.trash?'Удалённые операции можно восстановить или удалить навсегда. Окончательное удаление необратимо.':'Полная история доходов, расходов и переводов между супругами.'}</div></div><div class="journal-head-actions">${f.trash?trashActions:`<button class="btn btn-soft" id="exportCsv">CSV</button><button class="btn btn-soft" id="exportExcel">Excel</button><button class="btn btn-primary" id="newOperation">+ Новая операция</button>`}</div></div>
...
function journalTable(tx,trash=false){
  ...
  return `<div class="journal-table"><div class="journal-row journal-header"><div>Дата</div><div>Кто</div><div>Категория</div><div>Комментарий</div><div class="journal-amount">Сумма</div><div></div></div>${tx.map(x=>{
    const amount=transactionAmountText(x),tone=transactionTone(x.type);
    const amountCell=trash?`<div class="journal-amount ${tone}">${amount}</div>`:`<button type="button" class="journal-amount journal-amount-edit amount-edit-trigger ${tone}" data-id="${x.id}" title="Исправить сумму"><span>${amount}</span><small>Исправить</small></button>`;
    ...
    const category=x.type==='transfer'?'<b>Перевод</b><small>Не влияет на доходы/расходы семьи</small>':`<b>${esc(catName(x.category_id))}</b>${subName(x.subcategory_id)?`<small>${esc(subName(x.subcategory_id))}</small>`:''}`;
    return `<div class="journal-row ${x.type==='transfer'?'transfer-row':''} ${trash?'is-trash':''}" data-id="${x.id}"><div class="journal-date"><b>${new Date(x.occurred_at).toLocaleDateString('ru-RU',{day:'2-digit',month:'short',year:'numeric'})}</b><small>${new Date(x.occurred_at).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}${trash&&x.deleted_at?` · удалено ${new Date(x.deleted_at).toLocaleDateString('ru-RU',{day:'2-digit',month:'short'})}`:''}</small></div><div class="journal-person-cell"><span class="journal-person ${x.type==='transfer'?'transfer-flow':''}">${esc(transactionPeopleText(x))}</span></div><div class="journal-category">${category}</div><div class="journal-comment">${x.description?esc(x.description):'<span class="muted">Без комментария</span>'}</div>${amountCell}<div class="tx-actions journal-actions">${actions}</div></div>`;
  }).join('')}</div>`;
}
