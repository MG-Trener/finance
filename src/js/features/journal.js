...
function journalTable(tx,trash=false){
  ...
  return `<div class="journal-table"><div class="journal-row journal-header"><div>Дата</div><div>Кто</div><div>Категория</div><div>Комментарий</div><div class="journal-amount">Сумма</div><div></div></div>${tx.map(x=>{
    ...
    const actions=trash?`<button class="btn btn-soft btn-small historyTx" data-id="${x.id}">История</button><button class="btn btn-primary btn-small restoreTx" data-id="${x.id}">Восстановить</button>`:`<button class="btn btn-soft btn-small historyTx" data-id="${x.id}">История</button><button class="btn btn-soft btn-small editTx" data-id="${x.id}">Изменить</button><button class="btn btn-danger btn-small deleteTx" data-id="${x.id}">В корзину</button>`;
