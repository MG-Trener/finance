...
function transactionList(tx,actions=false){
  ...
  return `<div class="list">${tx.map(x=>{const amount=`${x.type==='income'?'+':'−'} ${money(x.amount)}`,pending=x._offline?'<span class="offline-row-badge">Ожидает синхронизации</span>':'',amountMarkup=actions?`<button type="button" class="tx-amount amount-edit-trigger ${x.type==='income'?'positive':'negative'}" data-id="${x.id}" title="Исправить сумму"><span>${amount}</span><small>Исправить</small></button>`:`<div class="tx-amount ${x.type==='income'?'positive':'negative'}">${amount}</div>`;return `<div class="tx ${x._offline?'tx-pending':''}"><div class="tx-icon">${x.type==='income'?'↗':'↘'}</div><div><div class="tx-title">${esc(catName(x.category_id))}${subName(x.subcategory_id)?` · ${esc(subName(x.subcategory_id))}`:''}</div><div class="tx-meta">${esc(personName(x.person_id))} · ${new Date(x.occurred_at).toLocaleString('ru-RU',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})}${x.description?' · '+esc(x.description):''}${pending}</div></div>${amountMarkup}${actions?`<div class="tx-actions"><button class="btn btn-soft btn-small editTx" data-id="${x.id}">Изменить</button><button class="btn btn-danger btn-small deleteTx" data-id="${x.id}">В корзину</button></div>`:''}</div>`}).join('')}</div>`;
}
