...
function txPersonLabel(personId){return state.people.find(p=>p.id===personId)?.label||''}
function txAllowedSubcategories(categoryId,personId){
  const label=txPersonLabel(personId);
...
function transactionForm(tx=null){
  ...
  const amountMarkup=tx?amountField:`<div class="amount-transfer-row full">${amountField}<button type="button" class="transfer-entry-btn" id="openTransfer" aria-label="Перевод между супругами"><span class="transfer-entry-icon">↔</span><b>Перевод</b></button></div>`;
  const ending=tx?`${timeMarkup}${saveMarkup}`:`${saveMarkup}${timeMarkup}`;
  return `<form id="txForm" class="quick-form quick-form-v3" data-edit-id="${tx?.id||''}"><input type="hidden" id="txType" value="${type}"><div class="full segmented"><button type="button" data-type="expense" class="${type==='expense'?'active':''}">Расход</button><button type="button" data-type="income" class="${type==='income'?'active':''}">Доход</button></div>${amountMarkup}<div class="field full"><label>Кто</label>${personSwitch(person)}</div><input type="hidden" id="personId" value="${person||''}">${quick.length?`<div class="field full quick-pairs"><label>Недавние</label><div class="quick-pair-list">${quick.map(q=>`<button type="button" class="quick-pair" data-qcat="${q.category_id}" data-qsub="${q.subcategory_id||''}">${esc(catName(q.category_id))}${q.subcategory_id?` · ${esc(subName(q.subcategory_id))}`:''}</button>`).join('')}</div></div>`:''}<div class="field"><label>Категория</label><select id="categoryId">${cats.map(c=>`<option value="${c.id}" ${c.id===catId?'selected':''}>${esc(c.name)}</option>`).join('')}</select></div><div class="field"><label>Подкатегория</label><select id="subcategoryId" ${subs.length?'':'disabled'}>${subs.length?subs.map(s=>`<option value="${s.id}" ${s.id===subId?'selected':''}>${esc(s.name)}</option>`).join(''):'<option value="">Нет подкатегорий</option>'}</select></div><div class="field full"><label>Комментарий</label><input id="description" value="${esc(tx?.description||'')}" placeholder="Необязательно"></div>${ending}</form>`;
}

function txRefreshSubcategories(form,preferred=''){
  const category=form?.querySelector('#categoryId'),subcategory=form?.querySelector('#subcategoryId'),personId=form?.querySelector('#personId')?.value||state.selectedPersonId||'';
  if(!category||!subcategory)return;
  const subs=txAllowedSubcategories(category.value,personId);
  ...
  subcategory.disabled=false;subcategory.innerHTML=subs.map(s=>`<option value="${s.id}">${esc(s.name)}</option>`).join('');subcategory.value=subs.some(s=>s.id===current)?current:subs[0].id;
}
